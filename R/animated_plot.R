...
animate_fitness_history <- function(fitness_history) {
    ...
    animate_plot <- function(iter) {
        for (i in seq_len(iter)) {
            temp <- data.frame(
                Generation = c(seq_len(i), seq_len(i)),
                Variable = c(rep("mean", i), rep("best", i)),
                Fitness = c(
                    -fitness_history[[1]][seq_len(i)],
                    -fitness_history[[2]][seq_len(i)]
                )
            )
            pl <- ggplot2::ggplot(temp, ggplot2::aes(
                x = .data$Generation,
                y = .data$Fitness,
                group = .data$Variable,
                ...
                ggplot2::geom_hline(
                    yintercept = max(temp$Fitness),
                    linetype = "dashed"
                ) +
                ggplot2::annotate("text",
                    x = 1, y = max(temp$Fitness) + 2,
    ...
    animation::saveGIF(animate_plot(length(fitness_history[[1]])),
